...
router.get('/:cpf_professor/:cnpj_escola/turmas/:id_turma/notas/acessar', (req, res, next) => {
    ...
    var sql = "SELECT notas.matricula_aluno, notas.valor1, notas.valor2, notas.valor3, notas.media FROM (alunos_turmas INNER JOIN notas ON alunos_turmas.matricula = notas.matricula_aluno AND alunos_turmas.id_turma = ?);";
...
router.get('/:cpf_professor/:cnpj_escola/turmas/:id_turma/notas/atribuir', (req, res, next) => {
    let id_turma = req.params.id_turma;
    let cpf_professor = req.params.cpf_professor;
    var sql = "SELECT matricula, professores_disciplinas.id_disciplina as disciplina FROM ((alunos_turmas INNER JOIN turmas_disciplinas ON turmas_disciplinas.id_turma = alunos_turmas.id_turma AND alunos_turmas.id_turma = ?) INNER JOIN professores_disciplinas ON professores_disciplinas.id_disciplina = turmas_disciplinas.id_disciplina AND professores_disciplinas.cpf_professor = ?);";
